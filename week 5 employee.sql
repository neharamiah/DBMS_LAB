...
select max(en) from (select count(ename) as en from employee group by mgrno) as mgrcount;

select ename from employee e,assigned_to a
where e.empno=a.empno
and a.jobrole="Manager"
...
from employee e,assigned_to a where e.empno = a.empno and a.empno is null;

select p.pname, (select count(*) from assigned_to a where a.pno = p.pno) from project p;

select deptno, avg(sal) as avg_salary, max(sal) from employee group by deptno;

select empno, sum(incentiveamt)from incentives group by empno;

select empno, ename from employee where empno in (select empno from assigned_to where pno = (select pno from project where pname = 'mobile app'));

select deptno, dname, dloc from dept where deptno not in (select deptno from employee);

select mgrno, count(*) as report_count from employee where mgrno is not null group by mgrno;

select empno from assigned_to group by empno having count(pno) > 1;
